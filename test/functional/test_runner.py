...
def main():
    ...
    tmpdir = "%s/alqo_test_runner_%s" % (args.tmpdirprefix, datetime.datetime.now().strftime("%Y%m%d_%H%M%S"))
    os.makedirs(tmpdir)
    ...
    enable_alqod = config["components"].getboolean("ENABLE_ALQOD")

    if config["environment"]["EXEEXT"] == ".exe" and not args.force:
        # https://github.com/alqo/alqo/commit/d52802551752140cf41f0d9a225a43e84404d3e9
        # https://github.com/alqo/alqo/pull/5677#issuecomment-136646964
        print("Tests currently disabled on Windows by default. Use --force option to enable")
        sys.exit(0)

    if not (enable_wallet and enable_utils and enable_alqod):
        print("No functional tests to run. Wallet, utils, and alqod must all be enabled")
...
def run_tests(test_list, src_dir, build_dir, exeext, tmpdir, jobs=1, enable_coverage=False, args=[], combined_logs_len=0):
    # Warn if alqod is already running (unix only)
    try:
        if subprocess.check_output(["pidof", "alqod"]) is not None:
            print("%sWARNING!%s There is already a alqod process running on this system. Tests may fail unexpectedly due to resource contention!" % (BOLD[1], BOLD[0]))
    ...
    #Set env vars
    if "ALQOD" not in os.environ:
        os.environ["ALQOD"] = build_dir + '/src/alqod' + exeext
        os.environ["ALQOCLI"] = build_dir + '/src/alqo-cli' + exeext
...
class TestHandler:
    ...
    def __init__(self, num_tests_parallel, tests_dir, tmpdir, test_list=None, flags=None):
        ...
        self.flags = flags
        self.num_running = 0
        # In case there is a graveyard of zombie alqods, we can apply a
...
class RPCCoverage():
    ...
    commands per `alqo-cli help` (`rpc_interface.txt`).
