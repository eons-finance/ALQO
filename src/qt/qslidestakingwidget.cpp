...
QSlideStackedWidget::QSlideStackedWidget(QWidget *parent)
...
void QSlideStackedWidget::setVerticalMode(bool vertical) {
    m_vertical=vertical;
}

void QSlideStackedWidget::setSpeed(int speed) {
    m_speed = speed;
}

void QSlideStackedWidget::setAnimation(enum QEasingCurve::Type animationtype) {
    m_animationtype = animationtype;
}

void QSlideStackedWidget::setWrap(bool wrap) {
    m_wrap=wrap;
}
void QSlideStackedWidget::slideInNext() {
    int now=currentIndex();
    if (m_wrap||(now<count()-1))
        slideInIdx(now+1);
}
void QSlideStackedWidget::slideInPrev() {
    int now=currentIndex();
    if (m_wrap||(now>0))
        slideInIdx(now-1);
}

void QSlideStackedWidget::slideInIdx(int idx, enum t_direction direction) {
...
void QSlideStackedWidget::slideInWgt(QWidget * newwidget, enum t_direction direction) {
...
void QSlideStackedWidget::animationDoneSlot(void) {
