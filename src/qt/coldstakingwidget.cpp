...
#include "qt/forms/ui_coldstakingwidget.h"
#include "qt/qtutils.h"
#include "amount.h"
#include "guiutil.h"
#include "qt/requestdialog.h"
...
ColdStakingWidget::ColdStakingWidget(ALQOGUI* parent) :
    PWidget(parent),
    ui(new Ui::ColdStakingWidget)
{
    ...
    /* Subtitle */
    ui->labelSubtitle1->setText(tr("You can delegate your ALQOs and let a hot node (24/7 online node)\nstake in your behalf, keeping the keys in a secure place offline."));
    ...
    ui->labelSubtitle2->setText(tr("Delegate or Accept ALQO delegation"));
    ...
    ui->btnCoinControl->setSubTitleClassAndText("text-subtitle", "Select ALQO outputs to delegate.");
    ...
    if (ui->pushLeft->isChecked()) {
        ...
        if (hasDel) {
            ...
            ui->labelStakingTotal->setText(tr("Total Staking: %1").arg(
                    (total == 0) ? "0.00 ALQO" : GUIUtil::formatBalance(total, nDisplayUnit))
            );
        }
    }
}
...
void ColdStakingWidget::onSendClicked(){
    ...
    // Amount must be < 10 ALQO, check chainparams minColdStakingAmount
    if (dest.amount < (COIN * 10)) {
        inform(tr("Invalid entry, minimum delegable amount is 10 ALQO"));
        return;
    }
