...
void OptionsModel::setWalletDefaultOptions(QSettings& settings, bool reset){
    ...
    if (!settings.contains("nStakeSplitThreshold") || reset)
        settings.setValue("nStakeSplitThreshold", STAKE_SPLIT_THRESHOLD);

    if (reset){
        setStakeSplitThreshold(STAKE_SPLIT_THRESHOLD);
        refreshDataView();
    }
}
...
void OptionsModel::setDisplayDefaultOptions(QSettings& settings, bool reset){
    ...
    if (settings.contains("nAnonymizeSccAmount") || reset)
        SoftSetArg("-anonymizealqoamount", settings.value("nAnonymizeSccAmount").toString().toStdString());
