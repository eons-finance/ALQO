...
bool CMasternodeConfig::read(std::string& strErr)
{
    ...
    if (!streamConfig.good()) {
        ...
        if (configFile != NULL) {
            std::string strHeader = "# Masternode config file\n"
                                    ...
                                    "# Example: mn1 127.0.0.2:20480 93HaYBVUCYjEMeeH1Y4sBGLALQZE1Yc1K64xiqgX37tGBDQL8Xg 2bcd3c84c84f87eaa86e4e56834c92927a07f9e18718810b92e0d0324456a67c 0"
                                    "#\n";
    ...
    for (std::string line; std::getline(streamConfig, line); linenumber++) {
        ...
        // this code can work fine for both scenarios
        if (port != Params().GetDefaultPort()) {
            strErr = _("Invalid port detected in masternode.conf") + "\n" +
                     strprintf(_("Line: %d"), linenumber) + "\n\"" + line + "\"" + "\n" +
                     strprintf(_("(must be %d for this network)"), Params().GetDefaultPort());
            streamConfig.close();
            return false;
        }
